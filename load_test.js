...
export default function () {
	...
	let sellRes = http.post(
		"http://localhost:8000/api/v1/saga/compra",
		JSON.stringify(data),
		{
			headers: { "Content-Type": "application/json" },
		}
	...
	let checkRes = http.get(
		"http://localhost:5000/api/v1/stock/check_quantity/2"
	);
...
export function teardown() {
	...
	products.forEach((product) => {
		let deleteRes = http.del(
			`http://localhost:5000/api/v1/stock/delete_product/${product.product_id}`
		);
