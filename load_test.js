...
export function setup() {
    ...
    let refuelRes = http.post('http://ms_stock.localhost/api/v1/stock/refuel', JSON.stringify({ product_id: 2, quantity: 1000 }), {
...
export default function () {
    ...
    let sellRes = http.post('http://ms_main_app.localhost/api/v1/saga/compra', JSON.stringify(data), {
        headers: { 'Content-Type': 'application/json' },
    });
    check(sellRes, { 'status was 200': (r) => r.status === 200 });

    // Chequear la cantidad de stock después de cada venta
    let checkRes = http.get('http://ms_stock.localhost/api/v1/stock/check_quantity/2');
...
export function teardown() {
    ...
    products.forEach(product => {
        let deleteRes = http.del(`http://ms_stock.localhost/api/v1/stock/delete_product/${product.product_id}`);
